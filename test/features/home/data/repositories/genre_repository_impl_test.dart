...
void main(){
  ...
  final tListGenreModel = [
    GenreModel(name: 'comedy', id: 1)
  ];
  ...
  group('getGenres', (){
    test(
      ...
      () async {
        ...
        when(()=>mockGenreRemoteDataSource.getGenres()).thenAnswer((_) async => tListGenreModel);
        when(() => mockGenreLocalDatasource.cachedGenres(tListGenreModel)).thenAnswer((invocation)  async => Future.value());
        //act
    ...
    group('device is online', (){
      ...
      test(
        ...
        () async {
          ...
          verify(() => mockNetworkInfo.isConnected).called(1);
          verify(() => mockGenreRemoteDataSource.getGenres()).called(1);
          verify(() => mockGenreLocalDatasource.cachedGenres(tListGenreModel)).called(1);
    ...
    group('device is offline', (){
      ...
      test(
        ...
        () async {
          ...
          //assert
          verify(()=>mockNetworkInfo.isConnected).called(1);
          verify(()=>mockGenreLocalDatasource.getGenresFromLocalDatasource()).called(1);
      ...
      test(
        ...
        () async {
          //arrange
          when(() => mockGenreLocalDatasource.getGenresFromLocalDatasource()).thenThrow(CacheException());
          //act
          final result = await repository.getGenres();
